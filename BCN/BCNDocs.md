...
2. Create an `env.json` file in the ./data/ directory or use the Configuration Wizard built into the GUI.  
...
2. **(Optional for client mode only)** Make a new Telegram bot in [BotFather](https://t.me/botFather).  
Now it's time to fill in the env file! Here is a template:
```json  
{  
  "MAIN_USER_ID": "Your Telegram ID in int format (optional for client mode)",  
  "BOT_TOKEN": "Token from BotFather (optional for client mode)",  
    "email": {  
...
**Important note:** The Telegram settings (`MAIN_USER_ID` and `BOT_TOKEN`) are optional only if you plan to use BCN in client (suboard) mode. If you want to run BCN server, these fields are required.
...
Subject: <Sender>:<Destination>[#<Index>]:<Flag>
...
- **Subject** contains information about the sender, destination, and flag.
  - For broadcast messages, the destination includes an index (255.255.255#123)
...
###### Broadcast Messages
When sending messages to all clients in the network, the broadcast address `255.255.255` is used. To ensure each client processes a broadcast message only once, these messages include an index in the format `255.255.255#123` where 123 is a sequential number that increments with each broadcast.

- **New format:** `255.255.255#123` - Includes numeric index for tracking
- **Legacy format:** `255.255.255` - Tracked by Message-ID for backward compatibility

Broadcast messages are not deleted from the mailbox after processing, allowing all clients to receive them. Each client keeps track of which broadcast messages it has already processed to avoid duplicate processing.
